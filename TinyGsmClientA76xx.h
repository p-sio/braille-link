...
#include "TinyGsmBattery.tpp"// no bluetooth, SSL, [TCP], wifi
...
class TinyGsmA76xx : public TinyGsmModem<TinyGsmA76xx<modemType>>,
 ...
 protected:
  ...
  //added here
    // Non‑blocking: starts speaking, only waits for "OK"
  bool textToSpeechAsyncImpl(String& text, uint8_t mode) {
    thisModem().sendAT(GF("+CTTS="), mode, ',', '"', text, '"');
    return thisModem().waitResponse() == 1;
  }

  // Stop speaking now (module will later emit "+CTTS: 0")
  bool textToSpeechStopImpl() {
    thisModem().sendAT(GF("+CTTS=0"));
    return thisModem().waitResponse(2000L) == 1;
  }
  //til here
  ...
  thisModem().sendAT(GF("+CTTSPARAM="), volume, ",", sysvolume, ",", 
                    digitmode, ",", pitch, ",", speed, ",", digitreading);
  return thisModem().waitResponse() == 1;
} 
